...
async function main() {
  try {
    ...
    apps.forEach((app) => {
      if (!app.id) return;
      const messages = allMessages.filter((msg) => msg.appid === app.id).sort((a, b) => new Date(b.date) - new Date(a.date)); // descending by date
    ...
    for (const app of apps) {
      ...
      for (const msg of messagesToDelete) {
        try {
          ...
          console.error(`Failed to delete message ${msg.id}:`, err.response?.data || err.message);
        }
      }
    }
  } catch (err) {
